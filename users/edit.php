...
try {
    ...
    // Définir la table cible (à ajuster selon le contexte, par exemple 'users')
    $table_name = isset($params['table_name']) ? $params['table_name'] : 'users'; // Ajustez selon votre besoin
    $table_query = new TableQuery($table_name);

    // Décoder les paramètres condition et data
    $condition = json_decode($params["condition"], true);
    $data = json_decode($params["data"], true);
    $old_image_path = isset($params["old_image"]) ? $params["old_image"] : null;

    // Valider que condition et data sont présents
    if (empty($condition) || empty($data)) {
        $reponse["status"] = false;
        $reponse["erreur"] = "Condition et données de mise à jour sont requis";
        echo json_encode($reponse);
        exit;
    }
    ...
    // Construire la condition pour la mise à jour
